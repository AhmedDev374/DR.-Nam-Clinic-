import React from 'react';
import { ArrowRight, CheckCircle, Clock, Shield, Users, Award, Scissors } from 'lucide-react';
import { PageProps } from '../types';

const SurgeryPage: React.FC<PageProps> = ({ onBookingClick }) => {
  const procedures = [
    {
      title: 'Tooth Extraction',
      description: 'Safe removal of damaged or problematic teeth',
      duration: '30-60 minutes',
      price: 'From $150',
      features: ['Simple extractions', 'Surgical extractions', 'Wisdom teeth removal', 'Minimal discomfort']
    },
    {
      title: 'Dental Implants',
      description: 'Permanent tooth replacement with titanium implants',
      duration: '60-120 minutes',
      price: 'From $1,500',
      features: ['Titanium implants', 'Bone integration', 'Natural appearance', 'Long-lasting']
    },
    {
      title: 'Bone Grafting',
      description: 'Bone augmentation for implant placement',
      duration: '90-120 minutes',
      price: 'From $800',
      features: ['Bone regeneration', 'Implant preparation', 'Natural materials', 'Healing support']
    },
    {
      title: 'Oral Surgery',
      description: 'Complex surgical procedures for oral health',
      duration: '60-180 minutes',
      price: 'From $500',
      features: ['Tumor removal', 'Cyst treatment', 'Jaw surgery', 'Reconstructive surgery']
    }
  ];

  const surgicalTypes = [
    {
      title: 'Simple Extractions',
      description: 'Removal of visible teeth that can be easily accessed',
      recovery: '1-3 days',
      anesthesia: 'Local'
    },
    {
      title: 'Surgical Extractions',
      description: 'Removal of impacted or broken teeth requiring surgical access',
      recovery: '3-7 days',
      anesthesia: 'Local or IV'
    },
    {
      title: 'Wisdom Teeth Removal',
      description: 'Extraction of third molars, often impacted',
      recovery: '3-7 days',
      anesthesia: 'IV sedation'
    },
    {
      title: 'Dental Implants',
      description: 'Placement of titanium posts for tooth replacement',
      recovery: '3-6 months',
      anesthesia: 'Local or IV'
    }
  ];

  const process = [
    {
      step: 1,
      title: 'Pre-Surgical Consultation',
      description: 'Thorough examination and treatment planning',
      duration: '60 minutes'
    },
    {
      step: 2,
      title: 'Pre-Surgical Preparation',
      description: 'Medical history review and pre-operative instructions',
      duration: '30 minutes'
    },
    {
      step: 3,
      title: 'Surgery',
      description: 'Professional surgical procedure with anesthesia',
      duration: '30-180 minutes'
    },
    {
      step: 4,
      title: 'Recovery Monitoring',
      description: 'Post-surgical care and monitoring',
      duration: '30-60 minutes'
    },
    {
      step: 5,
      title: 'Follow-up Care',
      description: 'Healing monitoring and post-operative instructions',
      duration: '30 minutes'
    }
  ];

  const benefits = [
    {
      icon: Shield,
      title: 'Safe & Sterile',
      description: 'State-of-the-art surgical facilities and protocols'
    },
    {
      icon: Users,
      title: 'Expert Surgeons',
      description: 'Experienced oral and maxillofacial surgeons'
    },
    {
      icon: Clock,
      title: 'Quick Recovery',
      description: 'Minimally invasive techniques for faster healing'
    },
    {
      icon: Award,
      title: 'Excellent Results',
      description: 'High success rates and patient satisfaction'
    }
  ];

  const preSurgeryInstructions = [
    'Avoid eating or drinking 8 hours before surgery',
    'Arrange for someone to drive you home',
    'Stop smoking at least 24 hours before surgery',
    'Inform us of all medications you are taking',
    'Wear comfortable, loose-fitting clothing'
  ];

  const postSurgeryCare = [
    'Apply ice packs to reduce swelling',
    'Take prescribed medications as directed',
    'Avoid smoking and alcohol for 48 hours',
    'Eat soft foods and avoid hot liquids',
    'Keep the surgical area clean and dry'
  ];

  return (
    <div className="pt-16 lg:pt-20">
      {/* Hero Section */}
      <section className="relative bg-gradient-to-br from-dental-600 to-dental-800 text-white overflow-hidden">
        <div className="absolute inset-0 bg-black/20"></div>
        <div className="absolute inset-0">
          <img
            src="https://images.unsplash.com/photo-1588776814546-3a7a7a7a7a7a?auto=format&fit=crop&w=1920&q=80"
            alt="Dental surgery procedure"
            className="w-full h-full object-cover"
            loading="lazy"
          />
        </div>
        <div className="relative container-custom section-padding">
          <div className="max-w-4xl mx-auto text-center">
            <h1 className="text-4xl md:text-6xl font-bold mb-6">
              Surgery
            </h1>
            <p className="text-xl md:text-2xl mb-8 text-dental-100">
              Advanced oral surgeries performed with precision and care for optimal recovery
            </p>
            <button
              onClick={onBookingClick}
              className="btn-primary bg-white text-dental-600 hover:bg-dental-50 flex items-center justify-center space-x-2 mx-auto"
            >
              <span>Schedule Consultation</span>
              <ArrowRight className="w-4 h-4" />
            </button>
          </div>
        </div>
      </section>

      {/* Introduction */}
      <section className="bg-white py-20">
        <div className="container-custom">
          <div className="max-w-4xl mx-auto text-center">
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-6">
              Expert Oral Surgery Services
            </h2>
            <p className="text-xl text-gray-600 mb-8">
              Our experienced oral surgeons provide safe, effective surgical treatments using 
              the latest techniques and technology. We prioritize your comfort and safety throughout the entire process.
            </p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mt-12">
              <div className="text-center">
                <div className="text-3xl font-bold text-dental-600 mb-2">98%</div>
                <div className="text-gray-600">Success Rate</div>
              </div>
              <div className="text-center">
                <div className="text-3xl font-bold text-dental-600 mb-2">20+</div>
                <div className="text-gray-600">Years Experience</div>
              </div>
              <div className="text-center">
                <div className="text-3xl font-bold text-dental-600 mb-2">5000+</div>
                <div className="text-gray-600">Successful Surgeries</div>
              </div>
            </div>
          </div>
        </div>
      </section>

      {/* Procedures */}
      <section className="bg-gray-50 py-20">
        <div className="container-custom">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
              Our Surgical Procedures
            </h2>
            <p className="text-xl text-gray-600 max-w-3xl mx-auto">
              We offer a comprehensive range of oral surgical procedures
            </p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {procedures.map((procedure, index) => (
              <div key={index} className="card p-8">
                <div className="flex justify-between items-start mb-4">
                  <h3 className="text-2xl font-bold text-gray-900">{procedure.title}</h3>
                  <div className="text-right">
                    <div className="text-lg font-semibold text-dental-600">{procedure.price}</div>
                    <div className="text-sm text-gray-500">{procedure.duration}</div>
                  </div>
                </div>
                <p className="text-gray-600 mb-6">{procedure.description}</p>
                <ul className="space-y-2 mb-6">
                  {procedure.features.map((feature, featureIndex) => (
                    <li key={featureIndex} className="flex items-center space-x-2">
                      <CheckCircle className="w-4 h-4 text-dental-600" />
                      <span className="text-gray-600">{feature}</span>
                    </li>
                  ))}
                </ul>
                <button
                  onClick={onBookingClick}
                  className="w-full btn-primary"
                >
                  Schedule Consultation
                </button>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* Surgical Types */}
      <section className="bg-white py-20">
        <div className="container-custom">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
              Types of Oral Surgery
            </h2>
            <p className="text-xl text-gray-600 max-w-3xl mx-auto">
              We perform various types of oral surgical procedures
            </p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {surgicalTypes.map((type, index) => (
              <div key={index} className="card p-6">
                <h3 className="text-xl font-bold text-gray-900 mb-3">{type.title}</h3>
                <p className="text-gray-600 mb-4">{type.description}</p>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <span className="font-semibold text-gray-900">Recovery:</span>
                    <span className="text-gray-600 ml-2">{type.recovery}</span>
                  </div>
                  <div>
                    <span className="font-semibold text-gray-900">Anesthesia:</span>
                    <span className="text-gray-600 ml-2">{type.anesthesia}</span>
                  </div>
                </div>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* Process */}
      <section className="bg-dental-50 py-20">
        <div className="container-custom">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
              Your Surgical Journey
            </h2>
            <p className="text-xl text-gray-600 max-w-3xl mx-auto">
              We guide you through every step of your surgical experience
            </p>
          </div>
          <div className="max-w-4xl mx-auto">
            <div className="space-y-8">
              {process.map((step, index) => (
                <div key={index} className="flex items-start space-x-6">
                  <div className="flex-shrink-0">
                    <div className="w-12 h-12 bg-dental-600 text-white rounded-full flex items-center justify-center font-bold text-lg">
                      {step.step}
                    </div>
                  </div>
                  <div className="flex-grow">
                    <h3 className="text-xl font-semibold text-gray-900 mb-2">{step.title}</h3>
                    <p className="text-gray-600 mb-2">{step.description}</p>
                    <div className="text-sm text-dental-600 font-medium">{step.duration}</div>
                  </div>
                </div>
              ))}
            </div>
          </div>
        </div>
      </section>

      {/* Pre & Post Surgery Care */}
      <section className="bg-white py-20">
        <div className="container-custom">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
              Pre & Post Surgery Care
            </h2>
            <p className="text-xl text-gray-600 max-w-3xl mx-auto">
              Important instructions for optimal surgical outcomes
            </p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="card p-6">
              <h3 className="text-xl font-bold text-gray-900 mb-4">Before Surgery</h3>
              <ul className="space-y-2">
                {preSurgeryInstructions.map((instruction, index) => (
                  <li key={index} className="flex items-start space-x-2">
                    <CheckCircle className="w-4 h-4 text-dental-600 mt-1 flex-shrink-0" />
                    <span className="text-gray-600">{instruction}</span>
                  </li>
                ))}
              </ul>
            </div>
            <div className="card p-6">
              <h3 className="text-xl font-bold text-gray-900 mb-4">After Surgery</h3>
              <ul className="space-y-2">
                {postSurgeryCare.map((care, index) => (
                  <li key={index} className="flex items-start space-x-2">
                    <CheckCircle className="w-4 h-4 text-dental-600 mt-1 flex-shrink-0" />
                    <span className="text-gray-600">{care}</span>
                  </li>
                ))}
              </ul>
            </div>
          </div>
        </div>
      </section>

      {/* Benefits */}
      <section className="bg-gray-50 py-20">
        <div className="container-custom">
          <div className="text-center mb-16">
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-4">
              Why Choose Our Surgical Services
            </h2>
            <p className="text-xl text-gray-600">
              We provide safe, effective surgical care with excellent outcomes
            </p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {benefits.map(({ icon: Icon, title, description }, index) => (
              <div key={index} className="text-center">
                <div className="w-16 h-16 bg-dental-600 rounded-full flex items-center justify-center mx-auto mb-4">
                  <Icon className="w-8 h-8 text-white" />
                </div>
                <h3 className="text-xl font-semibold text-gray-900 mb-2">{title}</h3>
                <p className="text-gray-600">{description}</p>
              </div>
            ))}
          </div>
        </div>
      </section>

      {/* CTA Section */}
      <section className="bg-dental-600 py-20">
        <div className="container-custom text-center">
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">
            Ready for Oral Surgery?
          </h2>
          <p className="text-xl text-dental-100 mb-8 max-w-2xl mx-auto">
            Schedule a consultation with our oral surgeons to discuss your treatment options
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <button
              onClick={onBookingClick}
              className="btn-primary bg-white text-dental-600 hover:bg-dental-50 flex items-center justify-center space-x-2"
            >
              <span>Schedule Consultation</span>
              <ArrowRight className="w-4 h-4" />
            </button>
            <a
              href="tel:+1234567890"
              className="btn-secondary border-white text-white hover:bg-white hover:text-dental-600"
            >
              Call (123) 456-7890
            </a>
          </div>
        </div>
      </section>
    </div>
  );
};

export default SurgeryPage;
